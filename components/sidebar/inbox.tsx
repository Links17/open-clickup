      ...
      <Popover.Portal>
        ...
        >
          <div className="flex items-center justify-between border-b border-cu-border px-3 py-2">
            <span className="text-[13px] font-semibold">{t("nav.inbox")}</span>
            {unread > 0 && (
          ...
          <div className="max-h-[400px] overflow-y-auto">
            {items.length === 0 && (
              <div className="px-3 py-10 text-center text-[13px] text-cu-text-tertiary">{t("inbox.caughtUp")}</div>
            )}
            {items.map((n) => (
              <button
                key={n.id}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  <div className="text-[13px] leading-snug">
                    <span className="font-semibold">{n.actor?.name ?? t("inbox.someone")}</span> {n.body}
                  </div>
